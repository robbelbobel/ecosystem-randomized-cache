error id: file://<WORKSPACE>/core/src/main/scala/riscv/plugins/Cache.scala:local36
file://<WORKSPACE>/core/src/main/scala/riscv/plugins/Cache.scala
empty definition using pc, found symbol in pc: 
empty definition using semanticdb
empty definition using fallback
non-local guesses:
	 -riscv/increaseAgesUpTo.
	 -riscv/increaseAgesUpTo#
	 -riscv/increaseAgesUpTo().
	 -spinal/core/increaseAgesUpTo.
	 -spinal/core/increaseAgesUpTo#
	 -spinal/core/increaseAgesUpTo().
	 -spinal/lib/increaseAgesUpTo.
	 -spinal/lib/increaseAgesUpTo#
	 -spinal/lib/increaseAgesUpTo().
	 -increaseAgesUpTo.
	 -increaseAgesUpTo#
	 -increaseAgesUpTo().
	 -scala/Predef.increaseAgesUpTo.
	 -scala/Predef.increaseAgesUpTo#
	 -scala/Predef.increaseAgesUpTo().
offset: 16973
...
object EvictionPolicy extends SpinalEnum {
  val GRAN, GLRU, LRAN, LLRU = newElement() // Global Random, Global Least Recently Used, Local Random, Local Least Recently Used
}
...
private val cache =
      ...
      private def oldestWayLocal(skew: UInt, set: UInt): WayResult = {
        ...
        when(!(0 until ways).map(i => cache(skew)(set)(i).valid).reduce(_ && _)) {
          // Find Free Way
          result.way := (0 until ways).map(i => U(i, log2Up(ways) bits)).reduceBalancedTree((a, b) => Mux(cache(skew)(set)(a).valid === False, a, b))
         
        } otherwise {
          // Find Oldest Way
          result.way := (0 until ways).map(i => U(i, log2Up(ways) bits)).reduceBalancedTree((a, b) => Mux(cache(skew)(set)(a).age > cache(skew)(set)(b).age, a, b))
        }
      ...
      /** Local Eviction **/
      private def increaseAgesUpToLocal(skew: UInt, set: UInt, oldest: UInt): Unit = {
        for (i <- 0 until ways) {
          when(cache(skew)(set)(i).age < oldest) {
            cache(skew)(set)(i).age := (cache(skew)(set)(i).age + 1).resized
          }
        }
      }

      private def decreaseAgesUntilLocal(skew: UInt, set: UInt, youngest: UInt): Unit = {
        for (i <- 0 until ways) {
      ...
      private def evictWayGRAN(): WayResult = {
        ...
        decreaseAgesUntilGlobal(cache(result.skew)(result.set)(result.way).age)
      ...
      private def evictWayLRAN(): WayResult = {
        ...
        decreaseAgesUntilLocal(result.skew, result.set, cache(result.skew)(result.set)(result.way).age)
      ...
      private def evictWayLLRU(): WayResult = {
        ...
        oldestWayLocal(lastInsertionSkew, lastInsertionSet)
      ...
      private def insertRspInCache(address: UInt): Unit = {
        ...
        when(hit.valid && cache(hit.payload.skew)(hit.payload.set)(hit.payload.way).valid) {
          ...
          if (insertionPolicy == InsertionPolicy.LRU) {
            if (evictionPolicy == EvictionPolicy.GLRU || evictionPolicy == EvictionPolicy.GRAN){
              // Global Eviction
              increaseAgesUpToGlobal(
        ...
        when(
          ...
          when(found) {
            if (evictionPolicy == E == ReplacementPolicy.LRU) {
