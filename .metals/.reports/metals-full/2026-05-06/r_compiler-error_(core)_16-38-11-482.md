...
### java.lang.StringIndexOutOfBoundsException: Range [6549, 6549 + -3) out of bounds for length 25016
    ...
    val cacheArea = pipeline plug new Area {
      ...
      private def getSkew(set: UInt): UInt = {
        ...
        if (skewApproach == SkewApproach.RS) {
          ...
          val usage = (0 until skews).map(i => (U(@@i, getSkewUsage(set, U(i, log2Up(skews) bits))))

          // Find skew with lowest usage
          val best = usage.reduceBalancedTree((a, b) => Mux(a._2 < b._2 , a, b))
