...
### scala.reflect.internal.FatalError: 
  ThisType(method getSetIndex) for sym which is not a class
...
== Source file context for tree position ==
    ...
    97 
    98   // get all address bits that determine whether two addresses fall into the same cache line
    99   private def getSignificantBits(address: UInt): UInt = {
   100     U(getTagBits(address) ## getSetIndex(address, _CURSOR_key))
   101   }
   102 
   103   private def connect(_s: Stage, internal: MemBus, external: MemBus): Unit = {
